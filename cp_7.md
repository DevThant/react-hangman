Given that you already have a `WebSocketService` in your project, we can leverage it to broadcast sync progress events and listen for them in `spinner.vue`. Here’s how you can integrate it:

### 1. Modify `sync.vue` to Emit WebSocket Messages

Update your `sync.vue` component to send sync progress updates using the existing `WebSocketService`.
...
Update your `spinner.vue` component to listen for messages from the `WebSocketService`.
